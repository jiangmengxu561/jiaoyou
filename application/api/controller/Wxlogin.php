...
class Wxlogin extends Api
{
    ...
    //
    // 无需登录的接口,*表示全部
    protected $noNeedLogin = ['weChatRegister', 'information', 'login'];
    // 无需鉴权的接口,*表示全部
    ...
    public function weChatRegister()
    {
        ...
        $app_id = 'wx87a26c7d5e32ee45';
        $app_secret = 'c75eb9bb5bceaf4c0b2d52c35759bc9b';
        //用户
        $openid_data = getopenid($data['code'], $app_id, $app_secret);
//        print_r($openid_data);die;

        if ($openid_data['code'] == 1) {
            $this->error($openid_data['msg']);
        }
        $user = Db::name('user')->where('openid', $openid_data['openid'])->find();
        if ($user) {
            ...
            $ret = $this->auth->register($post['username'], '888888', '', '', $post);
            if (!$ret) {
    ...
    public function information()
    {
        ...
        if (!empty($data['nickname'])) {
            ...
            $array['nickname'] = $this->createNickname();
        ...
        if (!empty($data['avatar'])) {
            ...
            $array['avatar'] =Config::get('site.avatar');

        }

        if (!empty($data['birth'])) {
            $bir = explode('-',$data['birth']);
            $array['age'] = $this->getAgeByBirth($bir[0],$bir[1],$bir[2]);
        }
